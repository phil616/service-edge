...
export default function FRPCNew() {
  ...
  const { data: usedPorts } = useQuery({
    queryKey: ['used-ports', selectedFrps],
    queryFn: () => frpsUsedPorts(selectedFrps),
          ...
          {(fields, { add, remove }) => (
            <>
              {fields.map(({ key, name, ...rest }) => (
                <ProxyRow key={key} name={name} rest={rest} form={form} usedPorts={usedPorts ?? []} onRemove={() => remove(name)} />
              ))}
...
function ProxyRow({ name, rest, form, usedPorts, onRemove }: any) {
