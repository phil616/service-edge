...
import { createFRPCHost } from '../api/client'
...
export default function FRPCNew() {
  ...
  return (
    <Card title={<Typography.Title level={4} style={{ margin: 0 }}>创建 FRPC 主机</Typography.Title>}>
      ...
      <Form form={form} layout="vertical" style={{ maxWidth: 520 }} initialValues={{ frp_version: 'v0.61.1' }} onFinish={onFinish}>
        <Form.Item name="name" label="主机名称" rules={[{ required: true }]}>
          <Input placeholder="例如 home-nas" />
        </Form.Item>
        <Form.Item name="frp_version" label="FRP 版本" extra="该主机上 frpc 二进制的版本，所有连接共用">
